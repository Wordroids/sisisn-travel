<x-app-layout>
    ...
    // Get all groups from the quotation members
    const memberGroups = @json($quotation->getUniqueGroupNames() ?? []);
    
    // Add "Ungrouped Members" to the list of groups if there are ungrouped members
    const hasUngroupedMembers = @json($quotation->hasUngroupedMembers() ?? false);
    if (hasUngroupedMembers) {
        memberGroups.unshift("Ungrouped Members");
    }

    const existingAccommodations = @json($quotation->accommodations->load('roomDetails', 'additionalRooms', 'allocatedGroups'));
    ...
    // Log the entire existingData to verify it contains allocated_groups
    console.log(`Card ${cardIndex} - Full accommodation data:`, existingData);
    
    // Extract allocated group names from the existing data
    let allocatedGroups = [];
    
    // Check if we have allocated groups data and extract the group names
    if (existingData && existingData.allocated_groups && Array.isArray(existingData.allocated_groups)) {
        console.log(`Found allocated_groups array with ${existingData.allocated_groups.length} items`);
        existingData.allocated_groups.forEach(group => {
            if (group && group.group_name) {
                allocatedGroups.push(group.group_name);
            }
        });
    }
    
    console.log(`Card ${cardIndex} - Extracted group names:`, allocatedGroups);
    
    // Create the group allocation section HTML
    let groupAllocationHtml = '';
    if (memberGroups.length > 0) {
        groupAllocationHtml = `
            <div class="mt-6 pt-6 border-t border-gray-200">
                <h4 class="text-md font-semibold text-gray-700 mb-3">Group Allocation</h4>
                <div class="group-allocation-container bg-white p-4 rounded-lg border border-gray-200 shadow-sm">
                    <p class="text-sm text-gray-600 mb-3">Select which groups will stay in this hotel:</p>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 group-checkboxes">`;
        
        // Add a checkbox for each group
        memberGroups.forEach((group, index) => {
            // Check if this group is in the allocatedGroups array
            const isChecked = allocatedGroups.includes(group) ? 'checked' : '';
            console.log(`Group ${group} checked: ${isChecked ? 'yes' : 'no'}`);
            
            groupAllocationHtml += `
                <div class="flex items-center">
                    <input type="checkbox" id="group_${cardIndex}_${index}" 
                           name="accommodations[${cardIndex}][groups][]" 
                           value="${group}" class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                           data-group-name="${group}"
                           data-allocated-groups="${allocatedGroups.join(',')}"
                           ${isChecked}>
                    <label for="group_${cardIndex}_${index}" class="ml-2 block text-sm text-gray-900">
                        ${group}
                    </label>
                </div>`;
        });
    ...
    `;
        ...
        // Add validation for group allocation - ensure at least one group is selected
        addGroupAllocationValidation(newCard);
        initializeCardEvents(newCard);
    }

    function addGroupAllocationValidation(card) {
        const groupCheckboxes = card.querySelectorAll('.group-checkboxes input[type="checkbox"]');
        if (groupCheckboxes.length === 0) return;
        
        // Add a hidden input to track if any group is selected
        const hiddenValidation = document.createElement('input');
        hiddenValidation.type = 'hidden';
        hiddenValidation.name = 'group_allocation_valid';
        hiddenValidation.value = 'false';
        hiddenValidation.required = true;
        hiddenValidation.setAttribute('data-validation-message', 'Please allocate at least one group to this hotel');
        card.querySelector('.group-allocation-container').appendChild(hiddenValidation);
        
        // Update validation status whenever checkboxes change
        groupCheckboxes.forEach(checkbox => {
            checkbox.addEventListener('change', updateGroupValidation);
        });
        ...
        function updateGroupValidation() {
            ...
            hiddenValidation.value = anyChecked ? 'true' : 'false';
            
            // Visual feedback
            const container = card.querySelector('.group-allocation-container');
            if (!anyChecked) {
                container.classList.add('border-red-300');
                
                // Add warning message if it doesn't exist
                if (!container.querySelector('.validation-warning')) {
                    const warning = document.createElement('p');
                    warning.className = 'validation-warning text-sm text-red-600 mt-2';
                    warning.textContent = 'Please allocate at least one group to this hotel';
                ...
                const warning = container.querySelector('.validation-warning');
    ...
    document.querySelector('form').addEventListener('submit', function(e) {
        ...
        const validationInputs = document.querySelectorAll('input[name="group_allocation_valid"]');
        
        let hasError = false;
        validationInputs.forEach(input => {
            if (input.value === 'false') {
                hasError = true;
                const container = input.closest('.group-allocation-container');
                container.classList.add('border-red-300');
                ...
                if (!container.querySelector('.validation-warning')) {
                    const warning = document.createElement('p');
                    warning.className = 'validation-warning text-sm text-red-600 mt-2';
                    warning.textContent = input.getAttribute('data-validation-message') || 'Please allocate at least one group to this hotel';
                    container.appendChild(warning);
                }
        ...
        // Check for group overlap (same group assigned to multiple hotels with overlapping dates)
        const overlappingGroups = findOverlappingGroupAllocations(allCards);
        if (overlappingGroups.length > 0) {
            e.preventDefault();
            
            let message = "The following groups have been assigned to multiple hotels with overlapping dates:\n\n";
            overlappingGroups.forEach(entry => {
                message += `- ${entry.group} (${entry.hotels.join(', ')})\n`;
            });
    ...
    // Function to detect overlapping date ranges for the same group in different hotels
    function findOverlappingGroupAllocations(cards) {
        const groupAllocations = [];
        const overlappingGroups = [];
        
        // Collect all group allocations with their date ranges
        cards.forEach((card, cardIndex) => {
            const hotelName = card.querySelector('.hotel-select').selectedOptions[0]?.textContent || `Hotel #${cardIndex + 1}`;
            const startDate = new Date(card.querySelector('.checkin-date').value);
            const endDate = new Date(card.querySelector('.checkout-date').value);
            
            // Skip invalid date ranges
            if (isNaN(startDate) || isNaN(endDate) || startDate >= endDate) return;
            
            card.querySelectorAll('.group-checkboxes input[type="checkbox"]:checked').forEach(checkbox => {
        ...
        // Group the allocations by group name
        const groupedAllocations = {};
        groupAllocations.forEach(allocation => {
            if (!groupedAllocations[allocation.group]) {
                groupedAllocations[allocation.group] = [];
            }
            groupedAllocations[allocation.group].push(allocation);
        });
        
        // Check for overlaps within each group
        for (const group in groupedAllocations) {
            const allocations = groupedAllocations[group];
                ...
                for (let i = 0; i < allocations.length; i++) {
                    for (let j = i + 1; j < allocations.length; j++) {
                        if (dateRangesOverlap(
                            allocations[i].startDate, allocations[i].endDate,
                            allocations[j].startDate, allocations[j].endDate
                        )) {
                            // This group has overlapping hotel stays
                            overlaps.push(allocations[i].hotel);
                            if (!overlaps.includes(allocations[j].hotel)) {
                                overlaps.push(allocations[j].hotel);
                            }
                        }
                    }
                }
