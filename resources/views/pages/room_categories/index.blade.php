            ...
            <div class="relative overflow-hidden bg-white shadow-md sm:rounded-lg">
                <div class="px-4 divide-y">
                    <div class="flex flex-col py-3 md:flex-row md:items-center md:justify-between">
                        ...
                        <a href="{{ route('room_categories.create') }}"
                        ...
                        <tbody>
                            @foreach ($roomCategories as $roomCategory)
                                <tr class="border-b hover:bg-gray-100">
                                    ...
                                    <td class="px-4 py-2">
                                        <a href="{{ route('room_categories.edit', $roomCategory) }}"
                                            class="text-yellow-600 hover:underline">Edit</a>
                                        <form action="{{ route('room_categories.destroy', $roomCategory) }}"
                                            method="POST" class="inline">
                                            @csrf @method('DELETE')
                                            <button type="submit" class="text-red-600 hover:underline"
                                                onclick="return confirm('Delete this room category?')">Delete</button>
                                        </form>
                                    </td>
                                </tr>
                            @endforeach
