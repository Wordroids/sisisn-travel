    ...
    <div class="max-w-6xl mx-auto mt-10 p-6 bg-white shadow-lg rounded-lg">
        ...
        <div class="bg-gray-50 p-6 rounded-lg shadow-sm mt-6">
            ...
            <div class="grid grid-cols-2 gap-4 mt-4">
                <p><strong class="text-gray-600">Quote Reference:</strong> <span class="text-blue-600 font-semibold">{{ $quotation->quote_reference }}</span></p>
                <p><strong class="text-gray-600">Booking Reference:</strong> <span class="text-green-600 font-semibold">{{ $quotation->booking_reference }}</span></p>
                ...
                <p><strong class="text-gray-600">Tour Date:</strong> {{ $quotation->start_date }} to {{ $quotation->end_date }} ({{ $quotation->duration }} days)</p>
                <p><strong class="text-gray-600">Currency:</strong> {{ $quotation->currency }} (Rate: <span class="text-red-500 font-semibold">{{ $quotation->conversion_rate }}</span>)</p>
                <p><strong class="text-gray-600">Markup Per Person:</strong> <span class="text-gray-800">{{ $quotation->markup_per_person }}</span></p>
            </div>
        </div>
                    ...
                    <tbody class="divide-y">
                        @foreach ($quotation->paxSlabs as $paxSlab)
                            <tr class="bg-gray-50 hover:bg-gray-100 transition">
                                ...
                                <td class="px-4 py-3 text-center font-semibold text-blue-600">{{ $paxSlab->vehicle_payout_rate }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
...
<div class="bg-white p-6 rounded-lg shadow-sm mt-6">
    ...
    @endforeach
                    ...
                    <tbody class="divide-y">
                        @foreach ($quotation->travelPlans as $travelPlan)
                            <tr class="bg-gray-50 hover:bg-gray-100 transition">
                                ...
                                <td class="px-4 py-3 text-center">{{ $travelPlan->start_date }} - {{ $travelPlan->end_date }}</td>
                                <td class="px-4 py-3 text-center">{{ $travelPlan->vehicleType->name }}</td>
                                <td class="px-4 py-3 text-center font-semibold text-red-500">{{ $travelPlan->mileage }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
        ...
        <div class="flex justify-between mt-6">
            <a href="{{ route('quotations.index') }}" class="bg-gray-500 text-white py-3 px-6 rounded-lg hover:bg-gray-600">⬅ Back to List</a>
