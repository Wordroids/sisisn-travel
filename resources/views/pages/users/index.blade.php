            ...
            <div class="relative overflow-hidden bg-white shadow-md sm:rounded-lg">
                <div class="px-4 divide-y">
                    ...
                    <div class="flex flex-col items-stretch justify-between py-4 space-y-3 md:flex-row md:items-center md:space-y-0">
                        <a href="{{ route('users.create') }}" class="flex items-center justify-center px-4 py-2 text-sm font-medium text-white rounded-lg bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 focus:outline-none">
                    ...
                    <table class="w-full text-sm text-left text-gray-500">
                        ...
                        <tbody>
                            ...
                            <tr class="border-b hover:bg-gray-100">
                                ...
                                <td class="px-4 py-2">
                                    <div class="flex items-center">
                                        <div class="w-3 h-3 mr-2 {{ $user->status === 'active' ? 'bg-green-500' : 'bg-red-500' }} border rounded-full"></div>
                                        {{ ucfirst($user->status) }}
                                    </div>
                                </td>

                                <!-- Last Login -->
                                <td class="px-4 py-2">{{ $user->email_verified_at ? $user->email_verified_at->format('d M Y') : 'Never' }}</td>

                                <!-- Actions -->
                                <td class="px-4 py-2 text-right">
                                    <a href="{{ route('users.show', $user) }}" class="text-blue-600 hover:underline mr-2">View</a>
                                    <a href="{{ route('users.edit', $user) }}" class="text-yellow-600 hover:underline mr-2">Edit</a>
