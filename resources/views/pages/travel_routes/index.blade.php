            ...
            <div class="relative h-full bg-white shadow-md sm:rounded-lg">
                <div class="px-4 divide-y">
                    <div class="flex flex-col py-3 md:flex-row md:items-center md:justify-between">
                        ...
                        <a href="{{ route('travel_routes.create') }}"
                            ...
                            @foreach ($travelRoutes as $travelRoute)
                                <tr class="border-b hover:bg-gray-100">
                                        ...
                                        @if ($featuredAsset)
                                            <div class="relative group">
                                                ...
                                                @if ($allAssets->count() > 1)
                                                    <span
                                                        class="absolute bottom-1 right-1 bg-black bg-opacity-50 text-white text-xs px-2 py-1 rounded-full">
                                                        {{ $allAssets->count() }}
                                                    </span>
                                                @endif
                                            </div>
                                        ...
                                        <div class="relative">
                                            <p class="flex items-center text-sm text-gray-500">
                                                ...
                                                @if ($travelRoute->description)
                                                    ...
                                                    <div data-popover id="desc-{{ $travelRoute->id }}" role="tooltip"
                                    ...
                                    <td class="px-4 py-3 space-x-2">
                                        <a href="{{ route('travel_routes.edit', $travelRoute) }}"
                                            class="text-yellow-600 hover:underline">Edit</a>
                                        <form action="{{ route('travel_routes.destroy', $travelRoute) }}"
                                            method="POST" class="inline">
                                            @csrf @method('DELETE')
                                            <button type="submit" class="text-red-600 hover:underline"
                                                onclick="return confirm('Delete this travel route?')">Delete</button>
                                        </form>
                                    </td>
                                </tr>
                            @endforeach
